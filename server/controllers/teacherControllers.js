...
export const getAttendances = async (req,res) => {
    try {
        ...
        const attendances = await Attendence.find({ teacherId }).populate("subjectId","subjectName").populate({ path: "studentIds", select: "rollNo userId createdAt", populate: { path: "userId", select: "name" } });
        if (!attendances) {
            return res.json({ success: false, message: "Something went wrong!" });
        }
