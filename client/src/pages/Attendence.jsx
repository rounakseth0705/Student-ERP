...
const Attendence = () => {
    ...
    return(
        <div>
            <img onClick={() => navigate("/student-dashboard")} src={homeIcon} alt="homeIcon" className="absolute left-7 top-6 w-6 h-6 cursor-pointer sm:w-8 sm:h-8 sm:top-5 sm:left-10"/>
            <h1 className="text-center mt-5 text-2xl text-blue-950 font-semibold sm:text-4xl">Attendance</h1>
            <div className="flex flex-col justify-between gap-3 mt-10 mx-10 rounded shadow-lg py-5 px-3 bg-gray-50 font-semibold sm:mx-20 md:mx-30 lg:mx-50 2xl:mx-80 sm:px-10 md:px-12 lg:px-15">
                <span className="flex justify-between items-center">
                    ...
                    <h1 className="sm:text-2xl">{(userIdentity.classesAttended/userIdentity.courseId.classesDelivered[userIdentity.semester-1])*100}%</h1>
                </span>
                <span className="flex flex-col py-3">
                    <span className="flex justify-between items-center px-2 sm:px-5">
                        <h1 className="text-sm sm:text-raw">Total Classes Delivered</h1>
                        <h1 className="text-sm sm:text-raw">{userIdentity?.courseId?.classesDelivered[semester.current-1]}</h1>
                    </span>
                    <span className="flex justify-between items-center px-2 sm:px-5">
