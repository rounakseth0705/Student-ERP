...
import { useContext, useState } from "react";
import { User, Lock, ChevronDown, Eye, EyeOff, Loader2 } from "lucide-react"; 
import { UserContext } from "../context/AuthContext.jsx";
import CreateFirstAdmin from "../components/CreateFirstAdmin.jsx";
import { useNavigate } from "react-router-dom";
            ...
            <div className="relative w-full max-w-md bg-slate-900/60 backdrop-blur-2xl border border-slate-700/40 p-8 sm:p-10 rounded-3xl shadow-[0_0_50px_-12px_rgba(0,0,0,0.6)] flex flex-col items-center z-10">
                ...
                <form onSubmit={handleLogin} className="w-full flex flex-col items-center space-y-5">
                    ...
                    <div className="group w-full flex items-center gap-3 bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3.5 focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:bg-slate-950/70 transition-all duration-300">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            onClick={() => setShowPassword(!showPassword)}
                            className="text-slate-500 hover:text-slate-300 active:text-blue-400 focus:outline-none transition-colors ml-auto cursor-pointer p-0.5 shrink-0 disabled:opacity-50"
                        >
                            {showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
                        </button>
                    </div>
                    
                    {/* Login Button with Dynamic Spinning Loader Indicator */}
                    <button 
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-3.5 px-6 rounded-xl transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 shadow-lg shadow-blue-900/30 hover:shadow-blue-500/20 cursor-pointer text-sm sm:text-base mt-2 tracking-wide flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                    >
                        {isLoading ? (
                            <>
                ...
                <div className="w-full flex flex-col mt-6 relative">
                    ...
                    <div className="relative w-full">
                        <select 
                            ...
                            value={role}
                            disabled={isLoading}
                            className="w-full p-3.5 pr-10 outline-none text-slate-200 bg-slate-950/50 border border-slate-800 rounded-xl cursor-pointer text-sm focus:border-blue-500 focus:bg-slate-950/80 appearance-none transition-all duration-300 font-medium disabled:opacity-50"
                        >
                            <option value="student" className="bg-slate-900 text-slate-200">Student</option>
                            <option value="teacher" className="bg-slate-900 text-slate-200">Teacher</option>
                            <option value="admin" className="bg-slate-900 text-slate-200">Admin</option>
                ...
                <div className="flex justify-center items-center gap-1.5 mt-8 text-xs sm:text-sm text-slate-400">
                    ...
                    <button 
                        disabled={isLoading}
                        onClick={() => navigate("forgot-password")} 
                        className="text-blue-400 font-semibold hover:text-blue-300 hover:underline active:text-blue-500 transition-colors bg-transparent border-none p-0 cursor-pointer disabled:opacity-50"
                    >
                        Reset here
                    </button>
                </div>
