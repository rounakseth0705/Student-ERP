...
import { useNavigate } from "react-router-dom";
import toast from "react-hot-toast";
// Imported Lucide Icons to match layout
import { GraduationCap, ArrowUpRight, Loader2 } from "lucide-react";

const TeacherSubjects = ({ isMarkingAttendance = false }) => {
    const { subjects, loading, isLoading } = useContext(TeacherDashboardContext);
    const navigate = useNavigate();
    const day = useRef(new Date().toLocaleDateString("en-US", { weekday: "short" }));
    ...
    const isFetching = loading || isLoading || !subjects;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 selection:bg-blue-600 selection:text-white">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {isFetching ? (
                    ...
                    <div className="col-span-full py-16 text-center text-sm text-slate-500 font-medium">
                        <div className="flex flex-col items-center justify-center space-y-3">
                            <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
                            <span className="text-xs text-slate-400">Loading assigned subjects...</span>
                        </div>
                    </div>
                    ...
                    <div className="col-span-full py-16 text-center text-sm text-slate-500 font-medium bg-[#090f1c]/20 border border-slate-800/60 rounded-2xl">
