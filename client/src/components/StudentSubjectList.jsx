...
import { StudentDashboardContext } from "../context/StudentDashboardContext.jsx";
import { UserContext } from "../context/AuthContext.jsx";
import { BookOpen, Hash, User, Layers, Loader2, Inbox } from "lucide-react";
...
const StudentSubjectList = ({ isNavigate = false }) => {
    ...
    const handleGetSubjects = async () => {
        if (userIdentity?.courseId?._id && userIdentity?.semester) {
            setIsLoading(true);
            try {
                await getSubjects(userIdentity.courseId._id, userIdentity.semester);
            } finally {
    ...
    return (
        <div className="w-full font-sans selection:bg-blue-600 selection:text-white px-4 sm:px-6 lg:px-8">
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20 bg-[#090f1c] border border-slate-800/80 rounded-2xl max-w-5xl mx-auto mt-6 sm:mt-10 space-y-3">
                    <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
                    <p className="text-sm text-slate-400 font-medium">Fetching subjects list...</p>
                </div>
            ) : (
                /* Optimized grid structural containment tailored to match our 2-column rule */
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 mt-6 sm:mt-10">
                    {subjects && subjects.length > 0 ? (
                        ...
                        <div className="col-span-full text-center py-16 bg-[#090f1c] border border-slate-800/80 rounded-2xl">
                            <div className="flex flex-col items-center justify-center gap-3">
                                <div className="p-3 bg-slate-900 rounded-2xl border border-slate-800 text-slate-500">
                                    <Inbox className="w-8 h-8" />
                                </div>
                                <h3 className="text-sm font-medium text-slate-300">No Subjects Enrolled</h3>
                                <p className="text-xs text-slate-500 max-w-xs mx-auto">
                                    There are no subjects registered for your current course and semester context.
                                </p>
                            </div>
                        </div>
                    )}
